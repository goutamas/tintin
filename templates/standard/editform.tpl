...
<div class="block_in_wrapper">
	...
	<div class="row">
		...
		<select name = "priority" id="priority" required = "0" realname = "{#priority#}">
			...
			<option value="{#low#}" {if $project.priority == #low#}selected{/if}>{#low#}</option>
			<option value="{#medium#}" {if $project.priority == #medium#}selected{/if}>{#medium#}</option>
			<option value="{#high#}" {if $project.priority == #high#}selected{/if}>{#high#}</option>
		</select>
	</div>
