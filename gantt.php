...
foreach($myprojects as $aproject)
{
	...
	$thepro = array("stage"=>$aproject["name"], "date_start"=>$aproject["startstring"], "date_end"=>$aproject["endstring"], "color"=>$colors[$myi], "ID"=>$aproject["ID"]);
...
$chart_date_end_by_project = date("U", $lastend  + 86400 + 86400);
if ($chart_date_end_min < $chart_date_end_by_project)
{
	$chart_date_end = $chart_date_end_by_project;
}
else
{
...
$chart_height_in_px_by_projects = ((count ($events_array) + 1) * 22) + 2;
if ($chart_height_in_px_min < $chart_height_in_px_by_projects)
{
	$chart_height_in_px = $chart_height_in_px_by_projects;
}
else
{
...
foreach ($events_array as $key => $value)
{
	...
	$event_display .= "
		<div class=\"event_text\" style=\"display:block; position:absolute; overflow:hidden; top:{$top}px; left:{$position_left}px; width:{$width}px; height:15px; z-index: 101;\" >
			&nbsp;&nbsp;<span style = \"color:white;\"><a href = \"manageproject.php?action=showproject&id=$value[ID]\" target=\"_parent\" style=\"vertical-align:middle; color:white; text-decoration:none;\" title = \"$value[description]\">$stage_name</a></span>
		</div>
